...
public class BoardPage {
    public static String pagingStr(int totalCount, int pageSize, int blockPage,
        ...
        if (pageTemp != 1) {
        	pagingStr += "<a href='" + reqUrl + "?pageNum=1" + searchParams + "'>[첫 페이지]</a>";
            pagingStr += "&nbsp;";
            pagingStr += "<a href='" + reqUrl + "?pageNum=" + (pageTemp - 1) + searchParams
                         + "'>[이전 블록]</a>";
        }
        ...
        while (blockCount <= blockPage && pageTemp <= totalPages) {
            if (pageTemp == pageNum) {
                ...
                pagingStr += "&nbsp;<a href='" + reqUrl + "?pageNum=" + pageTemp + searchParams
                             + "'>" + pageTemp + "</a>&nbsp;";
            }
            pageTemp++;
            blockCount++;
        ...
        if (pageTemp <= totalPages) {
            pagingStr += "<a href='" + reqUrl + "?pageNum=" + pageTemp + searchParams
                         + "'>[다음 블록]</a>";
            pagingStr += "&nbsp;";
            pagingStr += "<a href='" + reqUrl + "?pageNum=" + totalPages + searchParams
                         + "'>[마지막 페이지]</a>";
        }
